...
describe('normalizeList', () => {
  ...
  it('unwraps ok:data payloads recursively', () => {
    expect(normalizeList({ ok: true, data: { data: [4] } })).toEqual([4]);
  });
...
describe('nowSheetStr', () => {
  it('uses slash separated format', () => {
    const base = new Date('2024-01-01T00:00:00Z');
    expect(nowSheetStr(base)).toBe('2024/01/01 08:00:00');
  });
});

describe('fmtDate', () => {
  it('preserves sheet style values', () => {
    expect(fmtDate('2025/10/05')).toBe('2025/10/05');
  });

  it('normalizes dashed timestamps to sheet format', () => {
    expect(fmtDate('2025-10-05 12:34:56')).toBe('2025/10/05 12:34:56');
  });
});
